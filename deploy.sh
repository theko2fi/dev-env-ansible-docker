...
specifyNameServer(){
	...
	docker exec -ti $USER-debian-$i /bin/sh -c "echo 'nameserver 8.8.8.8' | sudo tee -a /etc/resolv.conf"
	docker exec -ti $USER-debian-$i /bin/sh -c "echo 'nameserver 8.8.4.4' | sudo tee -a /etc/resolv.conf"
}
